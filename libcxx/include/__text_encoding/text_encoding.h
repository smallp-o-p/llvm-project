...
struct text_encoding {
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr explicit text_encoding(string_view __enc) noexcept {
    if (auto __enc_d = __find_name(__enc)) {
  ...
  _LIBCPP_HIDE_FROM_ABI constexpr text_encoding(id __i) noexcept : __mib(__i), __name() {}

  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI constexpr id mib() const noexcept { return __mib; }
  [[nodiscard]] _LIBCPP_HIDE_FROM_ABI constexpr const char* name() const noexcept { return __name; }

  // [text.encoding.aliases], class text_encoding​::​aliases_view
  struct aliases_view;
  _LIBCPP_HIDE_FROM_ABI constexpr aliases_view aliases() const noexcept;
...
private:
  ...
  _LIBCPP_HIDE_FROM_ABI static constexpr const __encoding_data* __find_name(string_view __a) {
    auto __data_ptr = __enc_data + 2, __data_end = std::end(__enc_data) - 1;

    for (; __data_ptr != __data_end; __data_ptr++) {
